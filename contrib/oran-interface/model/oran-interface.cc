...
void E2Termination::DoStart ()
{
  ...
  // test - intead of running loop run directly the call back
...
E2Termination::ProcessRicControlHandoverRequest(E2AP_PDU_t* ric_ctlr_msg){

  std::map<long, std::map<long, long>> _ret_map;

  RICcontrolRequest_t ric_ctrl = ric_ctlr_msg->choice.initiatingMessage->value.choice.RICcontrolRequest;

  int count = ric_ctrl.protocolIEs.list.count;
  int size = ric_ctrl.protocolIEs.list.size;

  RICcontrolRequest_IEs_t **ies = (RICcontrolRequest_IEs_t**)ric_ctrl.protocolIEs.list.array;
  ...
  for (int i = 0; i < count; i++) 
  {
    ...
    switch(pres) 
    {
      case RICindication_IEs__value_PR_NOTHING:
      {
        break;
      }
      // IE containing the RIC Request ID
      ...
      case RICcontrolRequest_IEs__value_PR_RICcontrolHeader:
        {
          RICcontrolHeader_t ricHeader = next_ie->value.choice.RICcontrolHeader;
          size_t msg_size = ricHeader.size;
          uint8_t *buf = (uint8_t *)calloc(1,size);
          memcpy(buf, &ricHeader.buf, msg_size);
          break;
        }
      case RICcontrolRequest_IEs__value_PR_RICcontrolMessage:
        {
          // here we decode the ric control msg
          NS_LOG_DEBUG ("Processing RIC Control message payload");
          RICcontrolMessage_t ricCtrlMsg = next_ie->value.choice.RICcontrolMessage;
          size_t msg_size = ricCtrlMsg.size;
          uint8_t *buf = (uint8_t *)calloc(1,size);
          memcpy(buf, &ricCtrlMsg.buf, msg_size);
          // we need to pass to the decoder to decode it
          _ret_map = encoding::decode_handover_control_message(buf, msg_size);
          break;
        }
...
{
  // NS_LOG_FUNCTION("Sending pdu to xapp");
