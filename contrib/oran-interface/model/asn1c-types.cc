...
BufferString::BufferString (std::string value, size_t size)
{
  NS_LOG_FUNCTION (this);
  CreateBaseBufferString (size);
  memcpy (m_bufferString->buf, value.c_str (), size);
}

void BufferString::CreateBaseBufferString (size_t size)
{
  NS_LOG_FUNCTION (this);
  m_bufferString = (Buffer_String_t *) calloc (1, sizeof (Buffer_String_t));
  m_bufferString->buf = (uint8_t *) calloc (1, size);
  m_bufferString->size = size;
}

BufferString::BufferString (void *value, size_t size)
{
  NS_LOG_FUNCTION (this);
  CreateBaseBufferString (size);
  memcpy (m_bufferString->buf, value, size);

  // std::cout << "Inside buffer string " << size << std::endl;
  // for (uint32_t _ind= 0; _ind<size; ++_ind){
  //   std::cout << (uint32_t)*(m_bufferString->buf+_ind) << " ";
  // }
  // std::cout << std::endl;
}
...
std::string BufferString::DecodeContent(){
...
OctetString::OctetString (void *value, size_t size)
{
  ...
  // std::cout << "Inside octet string " << size << std::endl;
  // for (uint32_t _ind= 0; _ind<size; ++_ind){
  //   std::cout << (uint32_t)*(m_octetString->buf+_ind) << " ";
  // }
  // std::cout << std::endl;
}
...
V2XUserReceivedSciMessages::V2XUserReceivedSciMessages(){
  m_v2XUserReceivedSciMessages = (V2XSciMessageItemList_t *) calloc (1, sizeof (V2XSciMessageItemList_t));
}

void 
V2XUserReceivedSciMessages::addReceivedSciMessage(uint8_t* sciHeaderBuffer, uint32_t sciHeaderBufferLength, 
                              uint8_t* sciTagBuffer, uint32_t sciTagBufferLength, double rsrp)
{
  V2XSciMessageItem_t* sciMessageItem = (V2XSciMessageItem_t *) calloc (1, sizeof (V2XSciMessageItem_t));
  // Buffer_String_t * headerBufferString = (Buffer_String_t *) calloc (1, sizeof (Buffer_String_t));
  // headerBufferString->buf = (uint8_t *) calloc (1, sciHeaderBufferLength);
  // headerBufferString->size = sciHeaderBufferLength;
  // memcpy (headerBufferString->buf, sciHeaderBuffer, sciHeaderBufferLength);

  // Buffer_String_t * tagBufferString = (Buffer_String_t *) calloc (1, sizeof (Buffer_String_t));
  // tagBufferString->buf = (uint8_t *) calloc (1, sciTagBufferLength);
  // tagBufferString->size = sciTagBufferLength;
  // memcpy (tagBufferString->buf, sciTagBuffer, sciTagBufferLength);
  // sciMessageItem->header = headerBufferString;
  // sciMessageItem->tag = tagBufferString;

  // sciMessageItem->header.size = sciHeaderBufferLength;
  // sciMessageItem->header.buf = (uint8_t *) calloc (1, sciHeaderBufferLength);
  // memcpy (sciMessageItem->header.buf, sciHeaderBuffer, sciHeaderBufferLength);
  
  // sciMessageItem->tag.size = sciTagBufferLength;
  // sciMessageItem->tag.buf = (uint8_t *) calloc (1, sciTagBufferLength);
  // memcpy (sciMessageItem->tag.buf, sciTagBuffer, sciTagBufferLength);

  sciMessageItem->header.size = 0;
  sciMessageItem->tag.size = 0;

  sciMessageItem->rsrp = rsrp;

  ASN_SEQUENCE_ADD (&m_v2XUserReceivedSciMessages->v2XSciMessageSingleItem.list, sciMessageItem);
}
void 
V2XUserReceivedSciMessages::addReceivedSciMessage(V2XSciMessageItem_t* sciMessageItem)
{
  ASN_SEQUENCE_ADD (&m_v2XUserReceivedSciMessages->v2XSciMessageSingleItem.list, sciMessageItem);
}
...
{
  return m_v2XUserReceivedSciMessages;
}

V2XSciMessageItemList
V2XUserReceivedSciMessages::GetValue ()
{
  return *m_v2XUserReceivedSciMessages;
}
...
V2XPacketDelayIntervalWrap::V2XPacketDelayIntervalWrap (long lowerInterval, long upperInterval, long numberOfPackets, long reservationPeriodMs, long bufferSize){
  m_v2XPacketDelayInterval =
      (V2XPacketDelayInterval_t *) calloc (1, sizeof (V2XPacketDelayInterval_t));
  
  m_v2XPacketDelayInterval->lowerInterval = lowerInterval;
  m_v2XPacketDelayInterval->upperInterval = upperInterval;
  m_v2XPacketDelayInterval->numberOfPackets = numberOfPackets;
  m_v2XPacketDelayInterval->reservationPeriod = reservationPeriodMs;
  m_v2XPacketDelayInterval->bufferSize = bufferSize;
}

V2XPacketDelayInterval *
V2XPacketDelayIntervalWrap::GetPointer ()
{
  return m_v2XPacketDelayInterval;
}

V2XPacketDelayInterval
V2XPacketDelayIntervalWrap::GetValue ()
{
  return *m_v2XPacketDelayInterval;
}

// Buffer size
V2XHarqBufferSizeWrap::V2XHarqBufferSizeWrap (long harqId, long numberOfPackets, long bufferSize){
  m_v2XHarqBufferSize =
      (V2XHarqBufferSize_t *) calloc (1, sizeof (V2XHarqBufferSize_t));
  m_v2XHarqBufferSize->numberOfPackets = numberOfPackets;
  m_v2XHarqBufferSize->harqId = harqId;
  m_v2XHarqBufferSize->bufferSize = bufferSize;
}

V2XHarqBufferSize *
V2XHarqBufferSizeWrap::GetPointer ()
{
  return m_v2XHarqBufferSize;
}

V2XHarqBufferSize
V2XHarqBufferSizeWrap::GetValue ()
{
  return *m_v2XHarqBufferSize;
}

// user buffer
V2XBufferPacketDelayIntervalsWrap::V2XBufferPacketDelayIntervalsWrap (long rnti){
  m_v2XBufferPacketDelay = (V2XBufferPacketDelays_t *) calloc (1, sizeof (V2XBufferPacketDelays_t));
  m_v2XBufferPacketDelay->v2xNodeId = rnti;
}

void 
V2XBufferPacketDelayIntervalsWrap::AddPacketInterval (V2XPacketDelayInterval_t *v2XPacketDelayInterval){
  // adding data to the sequence
  ASN_SEQUENCE_ADD (&m_v2XBufferPacketDelay->v2XPacketDelayIntervalList.list, v2XPacketDelayInterval);
}

void 
V2XBufferPacketDelayIntervalsWrap::AddHarqBufferSize (V2XHarqBufferSize_t *v2xHarqBufferSize){
  // adding data to the sequence
  ASN_SEQUENCE_ADD (&m_v2XBufferPacketDelay->v2XHarqBufferSizeList.list, v2xHarqBufferSize);
}

V2XBufferPacketDelays *
V2XBufferPacketDelayIntervalsWrap::GetPointer ()
{
  return m_v2XBufferPacketDelay;
}

V2XBufferPacketDelays
V2XBufferPacketDelayIntervalsWrap::GetValue ()
{
  return *m_v2XBufferPacketDelay;
}
...
V2XUserBufferPacketDelays::V2XUserBufferPacketDelays (long rnti)
{
  m_v2xAllBufferPacketDelays = (V2XAllBufferPacketDelays_t *) calloc (1, sizeof (V2XAllBufferPacketDelays_t));
  m_v2xAllBufferPacketDelays->v2xNodeId = rnti;
}

void 
V2XUserBufferPacketDelays::AddBufferPacketDelays (V2XBufferPacketDelays_t *v2XBufferPacketDelays){
  ASN_SEQUENCE_ADD (&m_v2xAllBufferPacketDelays->V2XBufferPacketDelaysList.list, v2XBufferPacketDelays);
}

// V2XUserBufferPacketDelays::V2XBufferPacketDelays (V2XAllBufferPacketDelays_t *v2xAllBufferPacketDelays)
// {m_v2xAllBufferPacketDelays = v2xAllBufferPacketDelays;}

// V2XUserBufferPacketDelays::~V2XBufferPacketDelays (){}

V2XAllBufferPacketDelays *
V2XUserBufferPacketDelays::GetPointer ()
{
  return m_v2xAllBufferPacketDelays;
}

V2XAllBufferPacketDelays
V2XUserBufferPacketDelays::GetValue ()
{
  return *m_v2xAllBufferPacketDelays;
}
...
V2XAllUsersBufferPacketDelays::V2XAllUsersBufferPacketDelays ()
{
  m_v2xAllUsersBufferPacketDelays = (V2XAllUsersBufferDelayReports_t *) calloc (1, sizeof (V2XAllUsersBufferDelayReports_t));
}

void 
V2XAllUsersBufferPacketDelays::AddUserBufferPacketDelays (V2XAllBufferPacketDelays_t *v2XUserBufferPacketDelays)
{
  ASN_SEQUENCE_ADD (&m_v2xAllUsersBufferPacketDelays->list, v2XUserBufferPacketDelays);
}

V2XAllUsersBufferDelayReports *
V2XAllUsersBufferPacketDelays::GetPointer ()
{
  return m_v2xAllUsersBufferPacketDelays;
}

V2XAllUsersBufferDelayReports
V2XAllUsersBufferPacketDelays::GetValue ()
{
  return *m_v2xAllUsersBufferPacketDelays;
}

// single report containing delays values and all sci messges

V2XSingleReportWrap::V2XSingleReportWrap (double x, double y)
{
  m_v2xSingleUserReport = (V2X_Single_User_Report_t *) calloc (1, sizeof (V2X_Single_User_Report_t));
  m_v2xSingleUserReport->xPosition = x;
  m_v2xSingleUserReport->yPosition = y;
}

V2XSingleReportWrap::V2XSingleReportWrap (V2XAllBufferPacketDelays_t *v2XUserBufferPacketDelays, V2XSciMessageItemList_t *v2XUserReceivedSciMessages, double x, double y)
{
  m_v2xSingleUserReport = (V2X_Single_User_Report_t *) calloc (1, sizeof (V2X_Single_User_Report_t));
  // m_v2xSingleUserReport->userBufferDelayList = v2XUserBufferPacketDelays;
  m_v2xSingleUserReport->userReceivedSciMessages = v2XUserReceivedSciMessages;
  m_v2xSingleUserReport->userBufferDelayList = (*v2XUserBufferPacketDelays);
  // m_v2xSingleUserReport->userBufferDelayList.userReceivedSciMessages = (*v2XUserReceivedSciMessages);
  // m_v2xSingleUserReport->userReceivedSciMessages = (*v2XUserReceivedSciMessages);
  m_v2xSingleUserReport->xPosition = x;
  m_v2xSingleUserReport->yPosition = y;
}
...
void 
V2XSingleReportWrap::AddUserBufferPacketDelays (V2XAllBufferPacketDelays_t *v2XUserBufferPacketDelays){
  // m_v2xSingleUserReport->userBufferDelayList = v2XUserBufferPacketDelays;
  m_v2xSingleUserReport->userBufferDelayList = (*v2XUserBufferPacketDelays);
}

void 
V2XSingleReportWrap::AddUserReceivedSciMessageList (V2XSciMessageItemList_t *v2XUserReceivedSciMessages){
  m_v2xSingleUserReport->userReceivedSciMessages = v2XUserReceivedSciMessages;
  // m_v2xSingleUserReport->userReceivedSciMessages = (*v2XUserReceivedSciMessages);
  //  m_v2xSingleUserReport->userBufferDelayList.userReceivedSciMessages = (*v2XUserReceivedSciMessages);
}
...
void 
V2XSingleReportWrap::AddUserBufferAndSciMessageList (V2XAllBufferPacketDelays_t *v2XUserBufferPacketDelays, V2XSciMessageItemList_t *v2XUserReceivedSciMessages){
  m_v2xSingleUserReport->userBufferDelayList = (*v2XUserBufferPacketDelays);
  // m_v2xSingleUserReport->userBufferDelayList.userReceivedSciMessages = (*v2XUserReceivedSciMessages);
}
...
V2X_Single_User_Report *
V2XSingleReportWrap::GetPointer ()
{
  return m_v2xSingleUserReport;
}

V2X_Single_User_Report
V2XSingleReportWrap::GetValue ()
{
  return *m_v2xSingleUserReport;
}
...
MeasurementItem::MeasurementItem (std::string name, Ptr<OctetString> value)
  : MeasurementItem (name){
    NS_LOG_FUNCTION (this << name << "Octet sting" << value);
    this->CreateMeasurementValue (MeasurementValue_PR_valueOctetString);
    // m_measurementItem->pmVal.choice.valueOctetString = value->GetPointer ();
    // m_measurementItem->pmVal.choice.valueOctetString = value->GetValue() ();
}

MeasurementItem::MeasurementItem (std::string name, Ptr<BufferString> value)
  : MeasurementItem (name){
    NS_LOG_FUNCTION (this << name << "Octet sting" << value);
    this->CreateMeasurementValue (MeasurementValue_PR_valueOctetString);
    // m_measurementItem->pmVal.choice.valueOctetString = value->GetPointer ();
...
// MeasurementItem::MeasurementItem (std::string name, Ptr<V2XAllUsersBufferPacketDelays> value)
// : MeasurementItem (name){
//   // NS_LOG_FUNCTION (this << name << "Octet sting" << value);
//   this->CreateMeasurementValue (MeasurementValue_PR_valueV2XBufferDelay);
//     m_measurementItem->pmVal.choice.valueV2XBufferDelay = value->GetPointer ();
// }

// MeasurementItem::MeasurementItem (std::string name, Ptr<V2XUserReceivedSciMessages> value)
// : MeasurementItem (name){
//   // NS_LOG_FUNCTION (this << name << "Octet sting" << value);
//   this->CreateMeasurementValue (MeasurementValue_PR_valueV2XSciMessages);
//     m_measurementItem->pmVal.choice.valueV2XSciMessages = value->GetPointer ();
// }

MeasurementItem::MeasurementItem (std::string name, Ptr<V2XSingleReportWrap> value)
: MeasurementItem (name){
  // NS_LOG_FUNCTION (this << name << "Octet sting" << value);
  this->CreateMeasurementValue (MeasurementValue_PR_valueV2XSingleUserReport);
  m_measurementItem->pmVal.choice.valueV2XSingleUserReport = value->GetPointer ();
}
