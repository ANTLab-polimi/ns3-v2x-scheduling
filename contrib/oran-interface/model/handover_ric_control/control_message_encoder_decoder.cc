...
// std::map<long, std::vector<sctp_buffer_t>> 
// extract_scheduling_map(V2X_Scheduling_All_Users_t* v2XSchedulingAllUsersList){
//     std::map<long, std::vector<sctp_buffer_t>> allUsersScheduling;
    
//     for(int idx = 0; idx < v2XSchedulingAllUsersList->list.count; ++idx){
//         V2X_Scheduling_User_t* singleUserScheduling = (V2X_Scheduling_User_t*)v2XSchedulingAllUsersList->list.array[idx];
//         long v2xNodeId = (long)singleUserScheduling->v2xNodeId;
...
//             sctp_buffer_t data = sctp_buffer_t();
//             data.buffer = (uint8_t *) calloc(1, schedItem->nrSlotAllocBuffer.size);
//             // memcpy(data.buffer, buf, std::min(schedItem->nrSlotAllocBuffer.size, MAX_SCTP_BUFFER_CTRL));
//             memcpy (data.buffer, schedItem->nrSlotAllocBuffer.buf, schedItem->nrSlotAllocBuffer.size);
//             singleUserAllocations.push_back(data);
//         }
//         allUsersScheduling.insert(std::pair<long, std::vector<sctp_buffer_t>>(v2xNodeId, singleUserAllocations));
//     }
