...
namespace ns3 { 
  ...
  public:
    ...
    RicControlMessage (E2AP_PDU_t* pdu, uint8_t* remainingData, uint32_t remainingDataSize);
    ~RicControlMessage ();
    ...
    // std::map<long, std::vector<encoding::sctp_buffer_t>> m_schedulingList;
    std::map<long, std::map<long, encoding::dest_sched_t>> m_schedulingList;
    
    std::string m_plmnString;
    enum ControlMessageFormatType {ControlMessage_PR_controlMessage_Format1, 
                                  ControlMessage_PR_handoverMessage_Format,
                                  ControlMessage_PR_v2xScheduling_Format
                                  };
    ControlMessageFormatType m_messageFormatType {ControlMessage_PR_v2xScheduling_Format};

    struct SchedulingSingleMsg{
      std::string plmnString;
      std::map<long, std::map<long, encoding::dest_sched_t>> scheduleList;
      ControlMessageFormatType messageFormatType{ControlMessage_PR_v2xScheduling_Format};
    };

    std::vector<SchedulingSingleMsg> m_schedulingListMulti;

    uint32_t m_remaining_bytes{0};

    bool m_lastReport{false};
