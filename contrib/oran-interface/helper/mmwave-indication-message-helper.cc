...
// modified
void
MmWaveIndicationMessageHelper::AddDuUePmUsedResourcesItem (
  std::string ueImsiComplete, long ueNumUsedSymbols){
  Ptr<MeasurementItemList> ueVal = Create<MeasurementItemList> (ueImsiComplete);

  ueVal->AddItem<double> ("RRU.NrSymbolsUsedDl.UEID", ueNumUsedSymbols);

  m_msgValues.m_ueIndications.insert (ueVal);
}
void
MmWaveIndicationMessageHelper::AddDuCellPmUsedResourcesItem (
  std::string cellIdComplete, long ueNumUsedSymbols){
  Ptr<MeasurementItemList> cellVal = Create<MeasurementItemList> (cellIdComplete);

  cellVal->AddItem<double> ("RRU.NrSymbolsUsedDl", ueNumUsedSymbols);
