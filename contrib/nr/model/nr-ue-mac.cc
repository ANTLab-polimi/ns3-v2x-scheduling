...
void 
NrUeMac::V2xE2SchedulingTraceCallback(Ptr<NrUeMac> ueMacPtr, uint64_t ueId, NrSlGrantInfo nrSlGrantInfo, std::string plmnId){
  ueMacPtr->V2xE2Scheduling(ueId, nrSlGrantInfo);
}
void
NrUeMac::V2xE2Scheduling(uint64_t ueId, NrSlGrantInfo nrSlGrantInfo){
  // NS_LOG_FUNCTION(this << "imsi" << m_imsi << "sched dest"<< ueId);
  // std::cout << "V2xE2Scheduling: " << "imsi " << m_imsi << " sched dest "<< ueId << std::endl;
  // here we should store the scheduling data to the grant info, which shall be used by the mac for transmiting
  // m_grantInfo
  // NS_LOG_DEBUG("Scheduling info in slot: " << m_currentSlot);
  // not meant for this user
  if (m_imsi != ueId){
    return;
  }
  ...
  //   " " << sched.slPsschSymStart << " " << sched.slPsschSymLength);
  // }
  // adding the xapp received grant to the grant info vector
  // we iterate over all allocations and update the grant info
  for (std::set<ns3::NrSlSlotAlloc>::iterator nrSlSlotAllocIt = nrSlGrantInfo.slotAllocations.begin ();
      nrSlSlotAllocIt!= nrSlGrantInfo.slotAllocations.end (); ++nrSlSlotAllocIt){
    const auto itGrantInfo = m_grantInfoXapp.find (nrSlSlotAllocIt->dstL2Id);
    ...
    if (foundDest)
    {
      ...
      // we check whether there are sceduling of future slot, which are avaialable to the xapp grant as well
      // if yes we remove them and for the rest we update m_grantInfoXapp
      std::set<NrSlSlotAlloc>::iterator slotAllocGrantXappIt= itGrantInfo->second.slotAllocations.begin();
      while (slotAllocGrantXappIt!=itGrantInfo->second.slotAllocations.end()){
        if (nrSlSlotAllocIt->sfn < slotAllocGrantXappIt->sfn){
          // it means that the previous grants scheduled for future slots
...
NrUeMac::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrUeMac")
    ...
    // modified
    .AddAttribute ("E2DelayReportPeriodicity",
                    "The periodicity in milliseconds to report the packet delays in buffer to xapp",
                    UintegerValue (10),
                    MakeUintegerAccessor (&NrUeMac::SetE2PacketDelayReportSendindPeriodicity,
                                          &NrUeMac::GetE2PacketDelayReportSendindPeriodicity),
                    MakeUintegerChecker<uint16_t> (1))

    .AddTraceSource ("E2PacketDelayReport",
...
NrUeMac::NrUeMac (void) : Object ()
{
  ...
  // start the periodic event of sending the buffer status delay to the buffer
  // the latter shall agreggate and send the report to the xapp
  Simulator::Schedule (MilliSeconds (m_e2PacketDelaySendReportPeriodicity),
                                  &NrUeMac::SendE2BufferDelayReport, this);
}
...
NrUeMac::DoNrSlSlotIndication (const SfnSf& sfn)
{
  ...
  else if (m_slTxPool->GetNrSlSchedulingType () == NrSlCommResourcePool::ORAN_SELECTED){
    // scheduling as based on oran
    // we expect the grant info to have been sent by the xapp and it is already populated
    // we extract the allocations of the slot in question
    // we do not get all the scheduling sent from xapp immidiately to permit for updating
     //Do not ask for resources if no HARQ/Sidelink process is available
    m_grantInfo.clear();
    if (m_nrSlHarq->GetNumAvaiableHarqIds () > 0)
    {
      for (const auto &itDst : m_sidelinkTxDestinations)
      {
        const auto itGrantInfoXapp = m_grantInfoXapp.find (itDst.first);
        bool foundDest = itGrantInfoXapp != m_grantInfoXapp.end () ? true : false;
        // means there exist scheduling for the user coming from the xapp
        if (foundDest)
        {
          // we should check if there is a scheduling for the reference slot
          std::set<ns3::NrSlSlotAlloc>::iterator slotAllocIt = itGrantInfoXapp->second.slotAllocations.begin();
      ...
      if (itGrantInfo.second.slResoReselCounter != 0 && itGrantInfo.second.slotAllocations.begin ()->sfn == sfn)
        {
          ...
          psschStatsParams.txSci1A = currentGrant.txSci1A;
...
NrUeMac::DoReportNrSlPacketDelayStatus (uint16_t packetSeqNumber, uint64_t waitaingSince, uint32_t numPackets, uint32_t packetSize, uint16_t rnti_rlc, const NrSlMacSapProvider::NrSlReportBufferStatusParameters &params){
  ...
  if (packetSeqNumber == 0){
    // means we have packet send to mac, thus buffer reduced
    // in a single mac frame, there might be several rlc packets
    // thus delete only those that are inserted in the last mac frame
    uint32_t numEntriesToDelete = 0;
    
    if (m_packetDelayMap[rnti].m_sequenceNumberPacket.size() > numPackets){
      numEntriesToDelete = m_packetDelayMap[rnti].m_sequenceNumberPacket.size() - numPackets;
      m_packetDelayMap[rnti].m_sequenceNumberPacket.erase(m_packetDelayMap[rnti].m_sequenceNumberPacket.begin(), 
                                                        m_packetDelayMap[rnti].m_sequenceNumberPacket.begin() + numEntriesToDelete);
      m_packetDelayMap[rnti].m_waitingSincePacket.erase(m_packetDelayMap[rnti].m_waitingSincePacket.begin(), 
                                                        m_packetDelayMap[rnti].m_waitingSincePacket.begin() + numEntriesToDelete);
      m_packetDelayMap[rnti].m_packetSize.erase(m_packetDelayMap[rnti].m_packetSize.begin(), 
                                                m_packetDelayMap[rnti].m_packetSize.begin() + numEntriesToDelete);
      NS_LOG_DEBUG("Deleting: Rnti " << rnti << " num pack " << numPackets << " # entr to del " << numEntriesToDelete);
    }
  }else{
    // only one packet is inserted in the tx buffer
    m_packetDelayMap[rnti].m_waitingSincePacket.push_back(waitaingSince);
    m_packetDelayMap[rnti].m_sequenceNumberPacket.push_back(packetSeqNumber);
    m_packetDelayMap[rnti].m_packetSize.push_back(packetSize);
    m_packetDelayMap[rnti].reservationPeriod = m_pRsvpTx.GetMilliSeconds();
    m_packetDelayMap[rnti].harqIdBufferSizeMap = m_nrSlHarq->GetDestinationHarqMap(rnti);

    // params.lcid

    NS_LOG_DEBUG("Pushing: Rnti " << rnti << " waiting since " << waitaingSince << " pack size " << packetSize);
  }
}
// end modification
...
{
  // NS_LOG_FUNCTION (this<< "lcid" << +params.lcid);
...
void
NrUeMac::SetE2PacketDelayReportSendindPeriodicity (uint16_t e2PacketDelaySendReportPeriodicity)
{
  NS_LOG_FUNCTION (this);
  m_e2PacketDelaySendReportPeriodicity = e2PacketDelaySendReportPeriodicity;
}

uint16_t
NrUeMac::GetE2PacketDelayReportSendindPeriodicity () const
{
  NS_LOG_FUNCTION (this);
  return m_e2PacketDelaySendReportPeriodicity;
}
void
NrUeMac::SendE2BufferDelayReport(){

  // sending the report and rescheduling
  double x;
  double y;
  if (m_netDevice!=nullptr){
    Ptr<Node> node = DynamicCast<Node>(m_netDevice->GetNode());
  ...
  std::copy_if(m_packetDelayMap.begin(), m_packetDelayMap.end(),
            std::inserter(packetDelayMap, packetDelayMap.begin()),
            [](std::pair<uint16_t, ns3::PacketDelayStruct> a) { return a.second.hasChanged; }
            );
  m_e2PacketDelayReportTrace(m_rnti, packetDelayMap, x, y);
  // m_e2PacketDelayReportTrace(m_rnti, m_packetDelayMap, x, y);
...
void 
NrUeMac::SetNetDevice(Ptr<NetDevice> netdevice){
