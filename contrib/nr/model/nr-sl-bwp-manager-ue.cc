...
void
NrSlBwpManagerUe::DoReportNrSlPacketDelayStatus (uint16_t packetSeqNumber, uint64_t waitingSince, uint32_t numPackets, uint32_t queueSize, uint16_t rnti, const NrSlMacSapProvider::NrSlReportBufferStatusParameters &params)
{
  NS_LOG_FUNCTION (this);
  ...
  itSlLc = it->second.find (slLcIdentifier);
  NS_ASSERT_MSG (itSlLc != it->second.end (), "could not find Sap for SL LC id " << +params.lcid);
  itSlLc->second->ReportNrSlPacketDelayStatus (packetSeqNumber, waitingSince, numPackets, queueSize, rnti, params);
}
