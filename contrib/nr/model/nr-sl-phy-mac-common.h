...
// modified
struct PacketDelayStruct
  {
    
    uint32_t packetNumberInBuffer;
    uint32_t bufferSizeInBytes;
    uint64_t reservationPeriod;
    std::vector<uint64_t> m_waitingSincePacket;  
    std::vector<uint16_t> m_sequenceNumberPacket;
    std::vector<uint32_t> m_packetSize;
    // harq id map and size
