...
// std::map<uint32_t, std::map<uint8_t, PacketDelayStruct>> 
// NrSlUeMacHarq::FetPacketDelayStruct(){
//   std::map<uint32_t, std::map<uint8_t, PacketDelayStruct>> _map;

//   for (uint32_t _harqId = 0; _harqId < m_nrSlHarqPktBuffer.size(); ++_harqId){
//     // m_nrSlHarqPktBuffer.at(_harqId).dstL2Id
//     PacketDelayStruct _packetDelayStruct = PacketDelayStruct();
//     _packetDelayStruct.packetNumberInBuffer = m_nrSlHarqPktBuffer.at(_harqId).pktBurst->GetSize();
//     for (std::list<Ptr<Packet> >::const_iterator packetListIt = m_nrSlHarqPktBuffer.at(_harqId).pktBurst->GetPackets().begin(); 
//         packetListIt != m_nrSlHarqPktBuffer.at(_harqId).pktBurst->GetPackets().begin(); ++ packetListIt){
//       _packetDelayStruct.m_packetSize.push_back(packetListIt->GetSize());
//       _packetDelayStruct.m_waitingSincePacket.push_back(packetListIt->Get());
//     }
//   }
...
NrSlUeMacHarq::GetDestinationHarqMap(uint32_t dstL2Id){
  std::map<uint8_t, uint32_t> harqIdBufferSizeMap;
  for (uint8_t _harqId = 0; _harqId < m_nrSlHarqPktBuffer.size(); ++_harqId){
    if (m_nrSlHarqPktBuffer.at(_harqId).dstL2Id == dstL2Id){
      // getting packet size 
      harqIdBufferSizeMap[_harqId] = m_nrSlHarqPktBuffer.at(_harqId).pktBurst->GetSize();
    }
  }
  return harqIdBufferSizeMap;
}
