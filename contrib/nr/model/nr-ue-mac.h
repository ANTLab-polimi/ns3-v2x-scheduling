...
public:
  ...
  // modified
  void SetE2PacketDelayReportSendindPeriodicity (uint16_t e2PacketDelaySendReportPeriodicity);
  uint16_t GetE2PacketDelayReportSendindPeriodicity () const;

  // inserting the net device for the position
  void SetNetDevice(Ptr<NetDevice> netdevice);

  // get the callback 
  static void V2xE2SchedulingTraceCallback(Ptr<NrUeMac> ueMacPtr, uint64_t ueId, NrSlGrantInfo nrSlGrantInfo, std::string plmnId);
  void V2xE2Scheduling(uint64_t ueId, NrSlGrantInfo nrSlGrantInfo);
  // end modification
...
protected:
  ...
  void DoReportNrSlPacketDelayStatus (uint16_t packetSeqNumber, uint64_t waitaingSince, uint32_t numPackets, uint32_t queueSize, uint16_t rnti, 
...
private:
  ...
  // xapp grant info
  GrantInfo_t m_grantInfoXapp; //!< The map of grant info per destination layer 2 id
  ...
  std::map<uint16_t, PacketDelayStruct> m_packetDelayMap;
  uint16_t m_e2PacketDelaySendReportPeriodicity;

  TracedCallback<uint16_t, std::map<uint16_t, PacketDelayStruct>, double, double> m_e2PacketDelayReportTrace;
