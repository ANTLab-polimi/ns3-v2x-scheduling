...
NrSlSciF1aHeader::Print (std::ostream &os)  const
{
  ...
  //    << ", Index starting Subchannel ReTx2 " << +m_indexStartSbChReTx2;
    ...
    os << "" << +m_priority
     << "," << +m_mcs
     << "," << +m_slSciStage2Format
     << ","<< +m_slResourceReservePeriod
     << "," << +m_totalSubChannels
     << "," << +m_indexStartSubChannel
     << "," << +m_lengthSubChannel
     << "," << +m_slMaxNumPerReserve
     << "," << +m_gapReTx1
     << "," << +m_gapReTx2
     << "," << +m_indexStartSbChReTx1
     << "," << +m_indexStartSbChReTx2;
}

std::string
NrSlSciF1aHeader::GetString(){
     return std::to_string(m_totalSubChannels) + \
     "," + std::to_string(m_priority) + \
     "," + std::to_string(m_indexStartSubChannel) + \
     "," + std::to_string(m_lengthSubChannel) + \
     "," + std::to_string(m_mcs) + \
     "," + std::to_string(m_slResourceReservePeriod) + \
     "," + std::to_string(m_slMaxNumPerReserve) + \
     "," + std::to_string(m_slSciStage2Format) + \
     "," + std::to_string(m_indexStartSbChReTx1) + \
     "," + std::to_string(m_indexStartSbChReTx2) + \
     "," + std::to_string(m_gapReTx1) + \
     "," + std::to_string(m_gapReTx2);
}
