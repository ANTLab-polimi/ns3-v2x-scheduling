...
NrUePhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NrUePhy")
    ...
    .AddTraceSource ("SensingData",
                     "The trace which generates the data about the rsrp",
                     MakeTraceSourceAccessor (&NrUePhy::m_sensingDataTrace),
                     "ns3::NrUeMac::SensingData")
      ;
  return tid;
}
...
NrUePhy::PhyPscchPduReceived (const Ptr<Packet> &p, const SpectrumValue &psd)
{
 ...
 SensingData sensingData (m_currentSlot, sciF1a.GetSlResourceReservePeriod (),
  ...
  // modified
  // add data to trace which shall be sent to the Gnb
  // get mobility 
  Ptr<MobilityModel> mm = m_netDevice->GetNode()->GetObject<MobilityModel> ();
  m_sensingDataTrace(m_imsi, m_rnti, sciF1a, tag, rsrpDbm, mm->GetPosition().x, mm->GetPosition().y);
  // end modification
}
