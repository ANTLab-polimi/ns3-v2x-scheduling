...
NrHelper::GetTypeId (void)
{
  static TypeId
    ...
    .AddAttribute ("PlmnId",
                  ...
                  StringValue("111"),
                  MakeStringAccessor(&NrHelper::m_ltePlmnId),
                  MakeStringChecker()
                  )
    // end modification
    ;
                                      ...
                                      uint8_t numberOfStreams)
{
  ...
  if(m_e2mode_nr) {
        ...
        // modified
        // const std::string enb_id{m_ltePlmnId.back()};

        // const std::string enb_id{m_ltePlmnId.substr(m_ltePlmnId.size()-2)};
        const std::string enb_id{m_ltePlmnId.substr(m_ltePlmnId.size()-2)+"00"};
        // std::string plmnId = m_ltePlmnId;
        // end modification
        NS_LOG_DEBUG("Creating the E2 term to ric term: " << m_e2ip<< " " << m_e2port<< " in local port " << local_port << " with plmn " << plmnId);

        NS_LOG_DEBUG("enb_id " << enb_id);
