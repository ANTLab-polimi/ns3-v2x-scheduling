...
SlPowerOutputStats::SlPowerOutputStats (){
}
...
//                        "Run INTEGER NOT NULL);");
  ...
  SlPowerOutputStats::DeleteWhere (m_db, RngSeedManager::GetSeed (),
                                 RngSeedManager::GetRun(), 
                                //  static_cast<uint32_t> (MpiInterface::GetSystemId ()),
                                 tableName);
}

void SlPowerOutputStats::SavePower(const SfnSf &sfnSf, Ptr<const SpectrumValue> txPsd,
                                 [[maybe_unused]] const Time &t, uint16_t rnti, uint64_t imsi,
                                 uint16_t bwpId, uint16_t cellId)
{

  SlPowerResultCache c;
...
void
SlPowerOutputStats::EmptyCache()
{
  WriteCache ();
}

void
SlPowerOutputStats::DeleteWhere (SQLiteOutput *p, uint32_t seed,
...
void SlPowerOutputStats::WriteCache ()
{
  ...
  for (const auto & v : m_powerCache)
    {
      ...
      NS_ASSERT (ret);
      ret = m_db->Bind (stmt, 4, v.duration);
      NS_ASSERT (ret);
      ret = m_db->Bind (stmt, 5, v.rnti);
      NS_ASSERT (ret);
      ret = m_db->Bind (stmt, 6, static_cast<uint32_t> (v.imsi));
      NS_ASSERT (ret);
      ret = m_db->Bind (stmt, 7, v.bwpId);
      NS_ASSERT (ret);
      ret = m_db->Bind (stmt, 8, v.cellId);
      NS_ASSERT (ret);
      ret = m_db->Bind (stmt, 9, v.txPowerRb);
      NS_ASSERT (ret);
      ret = m_db->Bind (stmt, 10, v.txPowerTotal);
      NS_ASSERT (ret);
      ret = m_db->Bind (stmt, 11, v.rbNumActive);
      NS_ASSERT (ret);
      ret = m_db->Bind (stmt, 12, v.rbNumTotal);
      NS_ASSERT (ret);
