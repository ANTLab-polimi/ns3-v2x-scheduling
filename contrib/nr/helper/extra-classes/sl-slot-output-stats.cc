...
SlSlotOutputStats::SlSlotOutputStats ()
{
}

void
SlSlotOutputStats::SetDb (SQLiteOutput *db, const std::string & tableName)
{
  m_db = db;
...
//                        "Run INTEGER NOT NULL);");
  ...
  SlSlotOutputStats::DeleteWhere (m_db, RngSeedManager::GetSeed (),
...
void
SlSlotOutputStats::SaveSlotStats (const SfnSf &sfnSf,
                                ...
                                uint32_t availableSym, uint16_t bwpId, uint16_t cellId)
{
  ...
  if (m_slotCache.size () * sizeof (SlSlotCache) > 100) // 1000000
    {
      WriteCache ();
    }
}
...
void
SlSlotOutputStats::EmptyCache()
{
  WriteCache ();
}

void
SlSlotOutputStats::DeleteWhere (SQLiteOutput *p, uint32_t seed,
...
void SlSlotOutputStats::WriteCache ()
{
