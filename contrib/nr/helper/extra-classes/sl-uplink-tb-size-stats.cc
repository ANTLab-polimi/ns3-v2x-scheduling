...
SlUplinkTbSizeStats::SlUplinkTbSizeStats ()
{
}
...
//                        "Run INTEGER NOT NULL);");
  ...
  SlUplinkTbSizeStats::DeleteWhere (m_db, RngSeedManager::GetSeed (),
...
void
SlUplinkTbSizeStats::SaveUlTbSizeStats (uint64_t imsi, uint64_t tb)
{
	SlUplinkTbSizeCache c;
...
void
SlUplinkTbSizeStats::EmptyCache()
{
  WriteCache ();
}

void
SlUplinkTbSizeStats::DeleteWhere (SQLiteOutput *p, uint32_t seed,
...
void SlUplinkTbSizeStats::WriteCache ()
{
  bool ret = m_db->SpinExec ("BEGIN TRANSACTION;");
  for (const auto & v : m_uplinkTbSizeCache)
    {
      sqlite3_stmt *stmt;
