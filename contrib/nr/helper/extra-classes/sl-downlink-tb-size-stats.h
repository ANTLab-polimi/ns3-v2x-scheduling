/*
 * ef-downlink-tb-size.h
 *
 *  Created on: Nov 24, 2021
 *      Author: ugjeci
 */

#ifndef SL_CONTRIB_HELPER_DOWNLINK_TB_SIZE_STATS_H_
#define SL_CONTRIB_HELPER_DOWNLINK_TB_SIZE_STATS_H_
 ...
 * \see EmptyCache
 */
class SlDownlinkTbSizeStats
{
public:
  /**
   * \brief Constructor
   */
	SlDownlinkTbSizeStats ();
   ...
   * Seed/Run pair.
   */
  void SetDb (SQLiteOutput *db, const std::string& tableName = "dlTbSize", uint32_t writeSize = 100000);
   ...
   * \param [in] tb Tb size
   */
  void SaveDlTbSizeStats (uint64_t imsi, uint64_t tb);
...
private:
  ...
  std::vector<DlTbSizeCache> m_dlTbSizeCache;         //!< Result cache
...
#endif /* CONTRIB_ELEPHANT_FLOW_HELPER_EF_DOWNLINK_TB_SIZE_STATS_H_ */
