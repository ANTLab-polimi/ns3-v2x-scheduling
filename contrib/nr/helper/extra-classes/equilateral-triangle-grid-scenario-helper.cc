 ...
 *
 */
#include "equilateral-triangle-grid-scenario-helper.h"
...
EquilateralTriangleGridScenarioHelper::EquilateralTriangleGridScenarioHelper ()
{
...
// std::vector<double> EquilateralTriangleGridScenarioHelper::siteDistances {0,1,1,1,1,1,1,std::sqrt(3),std::sqrt(3),std::sqrt(3),std::sqrt(3),std::sqrt(3),std::sqrt(3),2,2,2,2,2,2};
// std::vector<double> EquilateralTriangleGridScenarioHelper::siteAngles {0,30,90,150,210,270,330,0,60,120,180,240,300,30,90,150,210,270,330};

std::vector<double> EquilateralTriangleGridScenarioHelper::siteDistances {0,1,1};
std::vector<double> EquilateralTriangleGridScenarioHelper::siteAngles {0,0,60};

/**
 * \brief Creates a GNUPLOT with the EquilateralTriangle deployment including base stations
 * (BS), their EquilateralTriangle cell areas and user terminals (UT). Positions and cell
 ...
 * \param cellRadius EquilateralTriangle cell radius in meters
 */
static void
PlotEquilateralTriangleDeployment (const Ptr<const ListPositionAllocator> &sitePosVector,
                         ...
                         double cellRadius)
{
  ...
  std::string topologyFileRoot = "./EquilateralTriangle-topology";
...
//  topologyOutfile << "set autoscale" << std::endl;
  ...
  uint16_t margin = (8 * cellRadius) + 1;  //!< This is the farthest EquilateralTriangle vertex from the cell center
...
void
EquilateralTriangleGridScenarioHelper::SetNumRings (uint8_t numRings)
{
...
double
EquilateralTriangleGridScenarioHelper::GetEquilateralTriangleCellRadius () const
{
  return m_equilateralTriangleRadius;
}

Vector
EquilateralTriangleGridScenarioHelper::GetEquilateralTriangleCellCenter (const Vector &sitePos,
                                                     uint16_t cellId) const
{
  ...
  case 3:
      switch (GetSectorIndex (cellId))
        {
        case 0:
          center.x += m_equilateralTriangleRadius * std::sqrt (0.75);
          center.y += m_equilateralTriangleRadius / 2;
          break;
        ...
        case 2:
          center.y -= m_equilateralTriangleRadius;
          break;
...
void
EquilateralTriangleGridScenarioHelper::CreateScenario ()
{
  m_equilateralTriangleRadius = m_isd / 3;
  
  m_bs.Create (m_numBs);
  m_ut.Create (m_numUt);

  NS_ASSERT (m_isd > 0);
  NS_ASSERT (m_numRings < 1);
  NS_ASSERT (m_equilateralTriangleRadius > 0);
  ...
  for (uint16_t cellId = 0; cellId < m_numBs; cellId++)
    {
      ...
      Vector cellCenterPos = GetEquilateralTriangleCellCenter (bsPos, cellId);
      bsCenterVector->Add (cellCenterPos);

      //What about the antenna orientation? It should be dealt with when installing the gNB
    }

  // To allocate UEs, I need the center of the equilateralTriangle cell.
  ...
  // Spread UEs inside the inner equilateralTriangle radius
  ...
  const double outerR = m_equilateralTriangleRadius * std::sqrt(3) / 2 - m_minBsUtDistance;
  ...
  PlotEquilateralTriangleDeployment (sitePosVector, bsCenterVector, utPosVector, m_equilateralTriangleRadius);

}

int64_t
EquilateralTriangleGridScenarioHelper::AssignStreams (int64_t stream)
{
