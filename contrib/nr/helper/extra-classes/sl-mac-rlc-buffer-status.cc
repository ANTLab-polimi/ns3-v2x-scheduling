...
SlMacBsrStats::SlMacBsrStats ()
{
}

void
SlMacBsrStats::SetDb (SQLiteOutput *db, const std::string & tableName, uint32_t writeSize)
{
  m_db = db;
...
//                        "Run INTEGER NOT NULL);");
  ...
  SlMacBsrStats::DeleteWhere (m_db, RngSeedManager::GetSeed (),
...
void
SlMacBsrStats::SaveSlMacBsrStats (SfnSf sfnSf, uint16_t cellId, uint16_t rnti, 
  ...
  uint16_t headoflinedelayrtx, uint8_t lcid)
{
  ...
  if (m_macBsrCache.size () * sizeof (MacBsrCache) > m_writeSize) // 1000000
    {
      WriteCache ();
    }
}
...
void
SlMacBsrStats::EmptyCache()
{
  WriteCache ();
}

void
SlMacBsrStats::DeleteWhere (SQLiteOutput *p, uint32_t seed,
...
void SlMacBsrStats::WriteCache ()
{
  ...
  for (const auto & v : m_macBsrCache)
    ...
    NS_ASSERT (ret);
    ret = m_db->Bind (stmt, 1, static_cast<uint32_t> (v.sfnSf.GetFrame ()));
    NS_ASSERT (ret);
    ret = m_db->Bind (stmt, 2, static_cast<uint32_t> (v.sfnSf.GetSubframe ()));
    NS_ASSERT (ret);
    ret = m_db->Bind (stmt, 3, static_cast<uint32_t> (v.sfnSf.GetSlot ()));
	  NS_ASSERT (ret);
