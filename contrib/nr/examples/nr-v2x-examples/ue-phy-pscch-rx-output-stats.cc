  ...
  for (const auto & v : m_pscchCache)
    {
      ...
      // modified
      NS_ABORT_UNLESS (ret);
      ret = m_db->Bind (stmt, 21, v.m_symStart);
      NS_ABORT_UNLESS (ret);
      ret = m_db->Bind (stmt, 22, v.m_numSym);
      // end modification
      NS_ABORT_UNLESS (ret);
      ret = m_db->Bind (stmt, 23, RngSeedManager::GetSeed ());
      NS_ABORT_UNLESS (ret);
      ret = m_db->Bind (stmt, 24, static_cast<uint32_t> (RngSeedManager::GetRun ()));
      NS_ABORT_UNLESS (ret);
