...
LteRlcUm::DoReportNrSlBufferStatus (void)
{
  ...
  // report the distribution of o packet delay as well
  // for (auto txBufferIt = m_txBuffer.begin(); txBufferIt!=m_txBuffer.end(); ++txBufferIt){
  // the delay of the packet
  // Time tempHolDelay = m_txBuffer.back().m_waitingSince;
  // uint32_t packetDelayGroupingGranularityIndex = (uint32_t)(tempHolDelay.GetMilliSeconds()/ m_packetDelayGroupingGranularity);
  // if (packetDelayGroupingGranularityIndex>=0){
  //   ++mValues[packetDelayGroupingGranularityIndex];
  // }
  // }
  // communicatin to mac
